...
use App\Domains\Match\Notifications\UpcomingMatchReminderNotification;
...
class FixtureCreatedListener implements ShouldQueue
{
    use InteractsWithQueue;

    public function handle(FixtureCreated $event): void
    {
        ...
        foreach ($teamIds as $teamId) {
            ...
            User::whereIn('id', $userIds)->get()->each(function (User $user) use ($event, $teamId) {
                $user->notify(new UpcomingMatchReminderNotification($event->match, (int) $teamId));
            });
        }
    }
}
