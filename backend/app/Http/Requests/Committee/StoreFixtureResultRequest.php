...
class StoreFixtureResultRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'scorers' => 'nullable|array|max:40',
            'scorers.*.team_id' => 'required|integer|exists:teams,id',
            'scorers.*.player_id' => 'nullable|integer|exists:players,id',
            'scorers.*.assist_player_id' => 'nullable|integer|exists:players,id',
            'scorers.*.minute' => 'required|integer|min:0|max:180',
            'scorers.*.half' => 'nullable|in:first,second',
            'scorers.*.period' => 'nullable|string|max:20',
            'scorers.*.type' => 'nullable|in:goal,own_goal,penalty_goal,assist',
            'cards' => 'nullable|array|max:40',
            'cards.*.team_id' => 'required|integer|exists:teams,id',
            'cards.*.player_id' => 'nullable|integer|exists:players,id',
            'cards.*.minute' => 'required|integer|min:0|max:180',
            'cards.*.half' => 'nullable|in:first,second',
            ...
            'events.*.half' => 'nullable|in:first,second',
    ...
    {
        return [
            'home_score.required' => 'نتيجة الفريق المضيف مطلوبة',
            'away_score.required' => 'نتيجة الفريق الضيف مطلوبة',
            'scorers.*.team_id.required' => 'يجب تحديد الفريق المسجل له الهدف',
            'cards.*.type.in' => 'نوع البطاقة غير صالح',
