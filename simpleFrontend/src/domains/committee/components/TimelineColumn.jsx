...
export default function TimelineColumn({ team, events, freshKey, onEdit, onDelete, t, halfDuration, readOnly = false }) {
    ...
    <div className="min-w-0">
      ...
      {sorted.length === 0 ? (
        ...
        <div className="xl:max-h-[42vh] xl:overflow-y-auto xl:pe-1">
          {sorted.map((ev, i) => (
            <EventRow key={ev._key} ev={ev} index={i} total={sorted.length} fresh={freshKey === ev._key} onEdit={() => onEdit(ev)} onDelete={() => onDelete(ev)} halfDuration={halfDuration} readOnly={readOnly} />
          ))}
        </div>
      )}
    </div>
