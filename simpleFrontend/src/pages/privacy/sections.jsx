...
export default function PrivacySections({ content }) {
  const { t } = useTranslation()
  const [activeId, setActiveId] = useState(content.sections[0]?.id)
  const observerRef = useRef(null)

  useEffect(() => {
    const headings = content.sections.map((s) => document.getElementById(s.id)).filter(Boolean)
      ...
      <div className="mx-auto grid max-w-[1200px] gap-8 px-6 lg:grid-cols-[280px,minmax(0,1fr)]">
        <aside className="hidden self-start lg:sticky lg:top-[88px] lg:block">
          <nav className="rounded-[26px] bg-white p-5 shadow-[0_1px_3px_rgba(15,23,42,0.06)] ring-1 ring-slate-200/60">
            <p className="mb-4 text-xs font-black uppercase tracking-wide text-slate-400">{t('privacy.page.tocTitle')}</p>
              ...
              {content.sections.map((s, i) => (
                <li key={s.id}>
                  <button
                    type="button"
                    onClick={() => jump(s.id)}
                    className={`group flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-start text-[13px] font-bold transition hover:bg-green-50 hover:text-green-700 ${
                      activeId === s.id
                  ...
                  >
                    <span className={`text-[10px] font-black ${
                      activeId === s.id ? 'text-green-500' : 'text-slate-300 group-hover:text-green-500'
                    }`}>{i + 1}</span>
                    {s.title}
                  </button>
                </li>
          ...
          <div className="rounded-[26px] bg-white p-6 shadow-[0_1px_3px_rgba(15,23,42,0.06),0_12px_32px_-16px_rgba(15,23,42,0.14)] ring-1 ring-slate-200/60 sm:p-8 lg:p-12">
            {content.sections.map((s, i) => (
              <section
                key={s.id}
                id={s.id}
                className={`${i > 0 ? 'border-t border-slate-100 pt-8' : 'pt-2'} ${i < content.sections.length - 1 ? 'pb-8' : ''}`}
              >
